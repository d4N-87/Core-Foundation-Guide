...
<script lang="ts">
	...
	// 1. Definiamo un tipo per i parametri della nostra azione per riutilizzarlo
	 ...
	 */
	// 2. Usiamo il tipo che abbiamo appena definito per il parametro iniziale
	function animateGrid(node: HTMLElement, params: AnimateGridParams) {
			...
			ctx = params.gsap.context(() => {
				...
				node.addEventListener('mousemove', (e: MouseEvent) => {
					...
					cards.forEach((card: any) => {
						if (card === activeCard) {
							params.gsap!.to(card, {
								x: 0, y: -10, scale: 1.1, duration: 0.4, ease: 'power2.out', overwrite: 'auto'
							});
							...
							params.gsap!.to(card, {
								x: targetX, y: targetY, scale: 1, duration: 0.4, ease: 'power2.out', overwrite: 'auto'
							});
				...
				node.addEventListener('mouseleave', () => {
					params.gsap!.to(cards, {
						x: 0, y: 0, scale: 1, duration: 0.6, ease: 'elastic.out(1, 0.75)',
		...
		return {
			// 3. LA CORREZIONE: Applichiamo il tipo anche al parametro 'newParams'
